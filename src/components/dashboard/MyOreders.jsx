...
const MyOreders = () => {
  ...
  const url = `https://resale-handing-server-side-770erufms-th-raju.vercel.app/bookings?email=${user.email}`;
  const {
    data: buyings = [],
    isLoading,
    refetch,
  ...
  const handleDelete = (buyings) => {
    ...
    if (agree) {
      fetch(
        `https://resale-handing-server-side-770erufms-th-raju.vercel.app/bookings/${buyings}`,
        {
          method: "DELETE",
        }
      )
  ...
  return (
    <div>
      <h3 className="text-4xl my-6 text-center font-bold">My Orders</h3>
          ...
          <tbody>
            {buyings
              ? buyings?.map((buying, i) => (
                  <tr key={buying._id} className="hover">
                    <th>{i + 1}</th>
                    <th>
                    ...
                    <td>
                      <button
                        onClick={() => handleDelete(buying._id)}
                    ...
                    <td>
                      {buying.price && !buying.paid && (
                        <Link to={`/dashboard/payment/${buying._id}`}>
